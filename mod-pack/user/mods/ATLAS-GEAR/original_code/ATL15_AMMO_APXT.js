class ATL15_AMMO_APXT
{
    constructor()
    {
        this.modname = "ATLAS_ATL15_AMMO_APXT";
    ...
    load()
    {
        const itemId = "010421_ATL15_AMMO_APXT00"; // unique item id, used in tarkov
        const itemClone = "5fd20ff893a8961fc660a954"; // base item, make clone from this

        // handbook
        const itemCategory = "5b47574386f77428ca22b33b"; // https://docs.sp-tarkov.com/#resources/items_stats.md -->> Weapon Parts & mod
        const itemFleaPrice = 1769; // Price of item on Fleamarket

        // item
        const itemPrefabPath = "ATL15_APXT.bundle"; // Server/mods/this.modname/bundles/... CAB 2e33dcd51900e1153e9615eca8d9afa3 === f2ba5588e4d8c01fb4f96ad9b2fb64bb
        const itemLongName = ".300 AAC Blackout APXT";
        const itemShortName = "APXT";
        const itemDescription = "Custom, .300 AAC Blackout Armor Piercing Explosive Tracer round. Manufactured by A T L A S.";

        // offer
        const itemTrader = "5935c25fb3acc3127c3d8cd9"; // PK
        const itemTraderPrice = 15;
    ...
    createItem(i_clone, i_id, i_path)
    {
        ...
        item._props.Damage = 20;
        item._props.PenetrationPower = 50;
        item._props.ArmorDamage = 88;
        item._props.ammoAccr = -5;
        item._props.ammoRec = 10;
        item._props.InitialSpeed = 808;

        // tracer
        item._props.Tracer = true;
        item._props.TracerColor = "tracerRed";
        item._props.TracerDistance = 0.5;

        // expolsive
        item._props.ExplosionStrength = 15;
        item._props.MinExplosionDistance = 0.15;
        item._props.MaxExplosionDistance = 1.9;
        item._props.HasGrenaderComponent = true;
        item._props.FragmentsCount = 4;
        item._props.FragmentType = "5943d9c186f7745a13413ac9"; // DMG = 20
...
module.exports.ATL15_AMMO_APXT = ATL15_AMMO_APXT;
