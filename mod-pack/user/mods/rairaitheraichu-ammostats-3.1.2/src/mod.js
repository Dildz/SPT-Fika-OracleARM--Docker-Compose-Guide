...
class AmmoStats {
    config;
    CalculateArmorLevel(penetrationValue, realismMode) {
        if (realismMode) {
    ...
    postDBLoad(container) {
        ...
        const itemTables = container.resolve("DatabaseServer").getTables().templates.items;
        ...
        let colorProfile = {};
        for (const itemID in itemTables) { // Iterate through all itemIDs
            if (itemTables[itemID]._props.ammoType === "bullet" || itemTables[itemID]._props.ammoType === "buckshot") {
                let bulletDamage = itemTables[itemID]._props.Damage; // Store its damage...
                let bulletPenetration = itemTables[itemID]._props.PenetrationPower; // Store its penetration...
                let bulletArmorTier = this.CalculateArmorLevel(bulletPenetration, realismMode); // Calculate what the best tier is for that penetration value
                let bulletType = "bullet";
                let bulletProjectiles = 1;
                if (itemTables[itemID]._props.ammoType === "buckshot") { // Override bulletType and bulletProjectiles if it's a buckshot type ammo
                    bulletType = "buckshot";
        ...
        for (const localeID in locales) { // Iterate through all language options
            ...
            for (const key in ammoStatDict) {
                ...
                try {
                    ...
                    if (this.config.debugLogging)
                        logger.success(`[AmmoStats]: Editing description for ${JSON.stringify(key)} in lang ${langText}`);
                }
                catch (exception) {
                    if (this.config.debugLogging)
                        logger.warning(`[AmmoStats]: WARNING! Error with item "${JSON.stringify(key)}" in lang ${langText}, skipping entry!`);
                    continue;
                }
                ...
                else if (ammoStatDict[key][3] === "buckshot" && this.config.addDamage === true) { // Damage string
                    stringToAdd = locDamage + ": " + ammoStatDict[key][0]
                ...
                if (this.config.addPen === true) { // Penetration string
                    stringToAdd += locPenetration + ": " + ammoStatDict[key][1]
                        + separatorChar;
                }
                if (this.config.addEffectArmorLv === true) { // Armor level string
                ...
                if (this.config.MODE.toLowerCase() === "prepend" && (this.config.addDamage || this.config.addPen || this.config.addEffectArmorLv)) { // Checking which mode was selected in the config
                ...
                else if (this.config.MODE.toLowerCase() === "prepend" && (this.config.addDamage || this.config.addPen || this.config.addEffectArmorLv)) { // Append mode
